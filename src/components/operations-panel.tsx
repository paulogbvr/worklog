...
import { CLIENT_STATUSES, type ClientStatusValue } from "@/lib/client-status";
import {
  buildPaymentMessage,
  PAYMENT_METHODS,
    ...
    <div className="fixed inset-0 z-[180] grid place-items-center bg-black/65 p-4 backdrop-blur-sm">
      ...
      >
        <div className="flex items-center justify-between border-b border-[color:var(--border)] px-5 py-4">
          <h3 className="font-semibold text-[color:var(--app-text-strong)]">
            {title}
          </h3>
          <button
            aria-label="Fechar"
...
export function OperationsPanel({
  ...
  const [receiptPreview, setReceiptPreview] = useState<DashboardPayment | null>(
    null,
  );
  const [receiptZoom, setReceiptZoom] = useState(100);
  const [invoicePreview, setInvoicePreview] = useState<DashboardPayment | null>(
    null,
  );
  const [invoiceZoom, setInvoiceZoom] = useState(100);
  const [workOperationDraft, setWorkOperationDraft] =
    useState<WorkOperationDraft>(createWorkOperationDraft);
  const [operationConfirmation, setOperationConfirmation] =
    useState<OperationConfirmation | null>(null);
  const [recordComposerOpen, setRecordComposerOpen] = useState(
    fixedView !== "records",
  );
  ...
  const [previewClient, setPreviewClient] = useState<DashboardClient | null>(
    null,
  );
  const [previewProject, setPreviewProject] = useState<DashboardProject | null>(
    null,
  );
  const [clientConfirmation, setClientConfirmation] =
  ...
  async function saveClient(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await readResponse(
        await fetch(
          clientDraft.id ? `/api/clients/${clientDraft.id}` : "/api/clients",
          {
      ...
      setClientDraft(null);
      toast({
        message: clientDraft.id
          ? "Alterações salvas."
          : "Cliente disponível para os projetos.",
  ...
  async function createShareLink() {
    ...
    try {
      ...
      setProjectDraft((current) =>
        current ? { ...current, sharePath: path } : current,
      );
      window.dispatchEvent(new Event("worklog-notifications-refresh"));
      toast({
        message:
          "O projeto agora possui uma visualização pública somente leitura.",
  ...
  async function disableShareLink() {
    ...
    try {
      ...
      setProjectDraft((current) =>
        current ? { ...current, sharePath: null } : current,
      );
      toast({
  ...
  function buildReminderText() {
    ...
    const fixedAmount = Number(
      projectDraft.reminderFixedAmount.replace(",", "."),
    );
    ...
    const dateLabel = projectDraft.reminderDueDate
      .split("-")
      .reverse()
      .join("/");
  ...
  function sendReminderWhatsApp() {
    ...
    if (!phone) {
      ...
      toast({
        message:
          "Cliente sem telefone. A mensagem foi copiada para envio manual.",
    ...
    <>
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button
                      ...
                      className="grid h-10 w-10 place-items-center rounded-md border border-[color:var(--border)] bg-[var(--surface)] text-[color:var(--text-muted)] transition-all duration-200 ease-out hover:border-[color:var(--border-strong)] hover:bg-[var(--hover-bg)] hover:text-[color:var(--app-text-strong)] active:scale-[0.98]"
                      onClick={() =>
                        setNotesProject({ id: project.id, name: project.name })
                      }
        ...
        {view === "records" ? (
          <>
                  ...
                  <div className="min-w-0">
                    <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-[color:var(--text-soft)]">
                          Pausas ficam separadas, mas a operação é salva como
                          uma unidade.
                        </p>
                      </div>
                    ...
                    <div className="space-y-3">
                      {workOperationDraft.intervals.map((interval, index) => {
                        ...
                        const setIntervalValue = (
                          ...
                          setWorkOperationDraft((current) => ({
                            ...current,
                            intervals: current.intervals.map(
                              (currentInterval) =>
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              ...
                              disabled={
                                workOperationDraft.intervals.length === 1
                              }
            ...
            <div className="rounded-lg border border-[color:var(--border)] bg-[var(--surface)] p-5">
              <h3 className="mb-1 text-base font-semibold text-amber-400">
                Histórico
              </h3>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center lg:flex-1">
                ...
                <label className="relative block w-full sm:w-52">
                  ...
                  <select
                    className={selectClass}
                    onChange={(event) =>
                      setClientSort(event.target.value as ClientSortValue)
                    }
                    value={clientSort}
                  >
            ...
            <div className="divide-y divide-[color:var(--border)] rounded-lg border border-[color:var(--border)] bg-[var(--surface)] px-5">
              {clients.length === 0 ? (
                ...
                <div className="py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-[color:var(--text-soft)]">
                    Ajuste a busca por “{clientSearch}” ou cadastre um novo
                    cliente.
                  </p>
                </div>
              ) : (
          ...
          <div className="space-y-4" id="pagamentos">
            <div className="rounded-lg border border-[color:var(--border)] bg-[var(--surface)] p-5">
              ...
              >
                <div>
                  ...
                  <p className="mt-1 text-sm text-[color:var(--text-soft)]">
                    O histórico atualiza imediatamente os valores recebidos e
                    pendentes.
                  </p>
                </div>
                <button
              ...
              {paymentComposerOpen ? (
                ...
                >
                    ...
                    <div className="block">
                      <span className="mb-1.5 flex items-center justify-between gap-3 text-xs text-[color:var(--text-muted)]">
                        ...
                        <span className="text-[color:var(--text-faint)]">
                          PDF, PNG, JPG, JPEG, WEBP · até 4 MB
                        </span>
                      </span>
                  ...
                  {paymentEditingId &&
                  payments.find((payment) => payment.id === paymentEditingId)
                    ?.hasReceipt ? (
                  ...
                  <div className="mt-5 rounded-md border border-[color:var(--border)] bg-[var(--surface-subtle)] p-4">
                    <button
                      ...
                      onClick={() =>
                        setPaymentInvoiceOpen((current) => !current)
                      }
                      role="switch"
                      type="button"
                    >
                    ...
                    {paymentInvoiceOpen ? (
                      <>
                        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,.8fr)]">
                          <label className="block">
                            ...
                            <input
                              className={fieldClass}
                              onChange={(event) =>
                                setPaymentInvoiceKey(event.target.value)
                              }
                                ...
                                <input
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if (file && file.size > 8 * 1024 * 1024) {
                                      setPaymentInvoice(null);
                                      setPaymentInvoiceInputKey(
                                        (current) => current + 1,
                                      );
                                      toast({
                                        message:
                                          "Selecione um arquivo de até 8 MB.",
                        ...
                        {paymentEditingId &&
                        payments.find(
                          (payment) => payment.id === paymentEditingId,
                        )?.hasInvoice ? (
                          <label className="mt-3 inline-flex items-center gap-2 text-xs text-[color:var(--text-muted)]">
                            <input
                              ...
                              onChange={(event) =>
                                setRemovePaymentInvoice(event.target.checked)
                              }
                              type="checkbox"
                            />
            ...
            <div className="rounded-lg border border-[color:var(--border)] bg-[var(--surface)] p-5">
              <div>
                ...
                <p className="mt-1 text-sm text-[color:var(--text-soft)]">
                  Edite dados, consulte comprovantes ou avise o cliente pelo
                  WhatsApp.
                </p>
              </div>
                  ...
                  payments.map((payment) => (
                    ...
                    >
                          ...
                          <div className="mt-2 rounded-md border border-sky-500/15 bg-sky-500/[0.05] p-2.5">
                            ...
                            {payment.hasInvoice ? (
                              <div className="mt-2 flex items-center gap-2">
                                ...
                                {payment.invoiceMimeType?.startsWith(
                                  "image/",
                                ) ||
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-2">
                        <strong className="mr-auto text-base lg:mr-2">
                          {payment.amountLabel}
                        </strong>
                        <button
      ...
      {projectDraft ? (
        <Modal
          ...
          title={
            projectDraft.clientId ? "Editar projeto" : "Configurar projeto"
          }
        >
          <form className="space-y-4 p-5" noValidate onSubmit={saveProject}>
            ...
            <div className="rounded-md border border-[color:var(--border)] bg-[var(--surface-subtle)] p-4">
              ...
              <span className="mt-1 block text-xs text-[color:var(--text-soft)]">
                Cobre por horas, por um preço fechado, ou marque como projeto
                próprio / sem fins lucrativos.
              </span>
                ...
                <label className="mt-4 block">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setProjectDraft((current) =>
                        current
                          ? { ...current, fixedPrice: event.target.value }
                          : current,
                      )
                    }
                  ...
                  />
                  <span className="mt-2 block text-xs text-[color:var(--text-faint)]">
                    Esse valor vira o total do projeto. Os valores por hora
                    acima continuam salvos e servem só como comparação interna.
                  </span>
                </label>
              ) : projectDraft.billingMode === "NON_PROFIT" ? (
                <p className="mt-4 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs text-[color:var(--text-soft)]">
                  Projeto próprio ou interno. As horas WakaTime, horas
                  dedicadas, notas, tarefas, histórico e link compartilhável
                  continuam funcionando — mas nada é cobrado e ele não entra em
                  valor pendente, a receber ou faturamento.
                </p>
              ) : null}
            </div>
            <div
            ...
            >
              ...
              >
                <span>
                  ...
                  <span className="mt-1 block text-xs text-[color:var(--text-soft)]">
                    Reuniões, pesquisa e outras operações manuais entram no
                    valor.
                  </span>
                </span>
                <span
              ...
              <label className="mt-4 block">
                  ...
                  onChange={(event) =>
                    setProjectDraft((current) =>
                      current
                        ? {
                            ...current,
                            dedicatedHourlyRate: event.target.value,
                          }
                        : current,
                    )
                  }
                ...
                />
                <span className="mt-2 block text-xs text-[color:var(--text-faint)]">
                  A tarifa fica salva, mas só é cobrada quando o controle acima
                  está ativo.
                </span>
              </label>
            </div>
                ...
                <div>
                  ...
                  {projectDraft.sharePath ? (
                    <p className="mt-2 text-xs text-[color:var(--text-muted)]">
                      ...
                      {projectDraft.shareAccessCount === 1
                        ? "acesso"
                        : "acessos"}
                    </p>
                  ) : null}
            ...
            <div className="rounded-md border border-[color:var(--border)] bg-[var(--surface-subtle)] p-4">
              ...
              >
                <span>
                  ...
                  <span className="mt-1 block text-xs text-[color:var(--text-soft)]">
                    Gera uma notificação interna na data e prepara a cobrança
                    pelo WhatsApp.
                  </span>
                </span>
                <span
                ...
                <div className="mt-4 space-y-4">
                          ...
                          onChange={(event) =>
                            setProjectDraft((current) =>
                              current
                                ? {
                                    ...current,
                                    reminderFixedAmount: event.target.value,
                                  }
                                : current,
                            )
                          }
                  ...
                  {!projectDraft.clientPhone ? (
                    <p className="text-xs text-[color:var(--warning-text)]">
                      O cliente não tem telefone cadastrado. A mensagem será
                      copiada para envio manual.
                    </p>
                  ) : null}
                  ...
                  <p className="text-xs text-[color:var(--text-faint)]">
                    O lembrete é salvo junto com o projeto e nunca envia nada
      ...
      {paymentConfirmation ? (
        ...
        >
          <div className="p-5">
            <p className="text-sm leading-6 text-[color:var(--text-muted)]">
              Excluir este pagamento recalculará o saldo do projeto. O
              comprovante relacionado também será removido do Storage quando
              estiver configurado.
            </p>
            <p className="mt-2 text-xs text-[color:var(--text-soft)]">
              ...
              <>
                <div className="mb-3 flex items-center justify-end gap-2">
                  <button
                    ...
                    onClick={() =>
                      setReceiptZoom((current) => Math.max(75, current - 25))
                    }
                    title="Reduzir"
                    type="button"
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setReceiptZoom((current) => Math.min(200, current + 25))
                    }
                    title="Ampliar"
                    type="button"
                  >
          ...
          <div className="space-y-4 p-5">
            ...
            <dl className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
              ...
              <DetailRow
                label="Telefone"
                value={
                  previewClient.phone ? formatPhone(previewClient.phone) : "—"
                }
              />
              <DetailRow
                label="CPF/CNPJ"
                value={
                  previewClient.taxId ? formatTaxId(previewClient.taxId) : "—"
                }
              />
              <DetailRow
              ...
              <DetailRow
                label="Cadastrado em"
                value={previewClient.createdAtLabel}
              />
              <DetailRow
                label="Endereço"
      ...
      {clientConfirmation ? (
        <Modal
          onClose={() => setClientConfirmation(null)}
        ...
        >
          <div className="space-y-4 p-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {previewProject.reminderEnabled &&
              previewProject.reminderStatusLabel ? (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[color:var(--border)] bg-[var(--surface-subtle)] px-2 py-0.5 text-xs text-[color:var(--text-muted)]">
            ...
            <dl className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
              <DetailRow
                label="Cliente"
                value={previewProject.clientName ?? "Sem cliente"}
              />
              <DetailRow
              ...
              <DetailRow
                ...
                value={previewProject.wakatimeLabel}
              />
              <DetailRow
                label="Horas dedicadas"
                value={previewProject.dedicatedLabel}
              />
              {previewProject.isNonProfit ? (
                ...
                <>
                  ...
                  <DetailRow
                    ...
                    value={previewProject.receivedValueLabel}
                  />
                  <DetailRow
                    label={
                      previewProject.pendingIsCredit
                        ? "Excedente"
                        : "Valor pendente"
                    }
              ...
              <DetailRow
                label="Criado em"
                value={previewProject.createdAtLabel}
              />
              ...
              <button
                ...
                onClick={() => {
                  const project = projects.find(
                    (item) => item.id === previewProject.id,
                  );
              ...
              <>
                <div className="mb-3 flex items-center justify-end gap-2">
                  <button
                    ...
                    onClick={() =>
                      setInvoiceZoom((current) => Math.max(75, current - 25))
                    }
                    title="Reduzir"
                    type="button"
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setInvoiceZoom((current) => Math.min(200, current + 25))
                    }
                    title="Ampliar"
                    type="button"
                  >
        ...
        >
          <form className="space-y-5 p-5" onSubmit={saveClient}>
            ...
            <label className="block">
              ...
              <span className="mt-2 block text-xs text-[color:var(--text-faint)]">
                Em automático, o status acompanha o vínculo: com projeto fica
                Ativo, sem projeto fica Sem projeto.
              </span>
            </label>
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setClientDraft((current) =>
                      current
                        ? { ...current, phone: formatPhone(event.target.value) }
                        : current,
                    )
                  }
                  inputMode="tel"
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="block">
                ...
                <input
                  aria-describedby={
                    clientTaxIdFeedback ? "tax-id-feedback" : undefined
                  }
                  className={[
                    fieldClass,
                    clientTaxIdFeedback
                  ...
                  onChange={(event) =>
                    setClientDraft((current) =>
                      current
                        ? { ...current, taxId: formatTaxId(event.target.value) }
                        : current,
                    )
                  }
